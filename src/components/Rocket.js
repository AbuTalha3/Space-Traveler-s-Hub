    ...
    <>
      <div className="single-rocket-div">
        ...
        <div className="single-rocket-texts">
          ...
          <button className={rocket.reserved ? 'activeButton' : 'notActiveButton'} type="button" onClick={() => dispatch(rocketsReducer({ rocket }))}>{rocket.reserved ? 'Cancel Reservation' : 'RESERVE'}</button>
        </div>
      </div>
    </>
  );
